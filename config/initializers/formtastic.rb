...
Formtastic::SemanticFormBuilder.optional_string = " <span style='font-size: small'>(optional)</span>"
...
# Changing the default checkbox layout.
class Formtastic::SemanticFormBuilder 
  def boolean_input(method, options) 
    ...
    input = self.check_box(method, strip_formtastic_options(options).merge(html_options), 
      options.delete(:checked_value) || '1', 
      options.delete(:unchecked_value) || '0') 
      options = options_for_label(options) 
    # the label() method will insert this nested input into the label at the last minute 
    # the line below is commented out here, but not in the original method. We don't want to pass the input to the label... 
    # options[:label_prefix_for_nested_input] = input 
    # however if we don't then attach the input directly it won't show up at all! So put << to concat the input to the output 
